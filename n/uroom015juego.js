...
function setup() {
 ...
 host.ball = host.ball || {
  ...
  var tinit = '<div style="font-size:10px;text-align:left;margin-left:5px;width:' + (pizarra.w - 10) + 'px;">' + fecha + '</div>'
...
function draw() {
  if (!iniciado) {
    ...
    if (shared.onoffProyector == 1) {
      //background(colorFondoT);
      clear()
      //if(partiNames[me.id].rol==2){
      //shared.seek=player.getCurrentTime()
      //timeproyector=shared.seek
      // }
      //console.log(shared.seek)
      //timeproyector=second()
      //shared.seek=timeproyector
      ...
      beginShape();
      vertex(200, 0);
      vertex(200, pizarra.h + 150);
      vertex(0, pizarra.h + 540);
      vertex(0, 0);
      endShape(CLOSE);
      fill(fondosc[basecolor - 1])
      beginShape();
      vertex(width - 50, 0);
      vertex(width, 0);
      vertex(width, pizarra.h + 540);
      vertex(width - 50, pizarra.h + 150);
      endShape(CLOSE);
    ...
    //if(partiNames[me.id].rol==2 && shared.modo=='video' && shared.modoestado>0 && isloadediframe==1){
    /*function isFunction(functionToCheck) {
     return functionToCheck && {}.toString.call(functionToCheck) === '[object Function]';
    }
    */
    //var funi= player && {}.toString.call(player) === '[object Function]';
    if (partiNames[me.id].rol == 2 && shared.modo == 'video' && shared.modoestado > 0 && isloadediframe == 1 && typeof(player) != "undefined") {

      shared.seek = player.getCurrentTime()
      //timeproyector=shared.seek
    }
    if (shared.modo == 'video' && shared.modoestado == 1) {
      // playVideo()
    }
    //if (second() % 4 == 0) { cargatexto();}
    //image(fondo,0,0,wim,him,host.ball['x'],host.ball['y'])
    //image(fondo,0,0,wim,him,host.ball['x'],host.ball['y'])
    //image(fondo, host.ball['x'], host.ball['y'], wim, him);
    // image(fondo, host.ball['x1'], host.ball['y'], wim, him);
    /* 
     if (partyIsHost()) {
     ...
     if(partiNames[me.id].rol=2){
          ...
          //if(mouseX>width*0.8){ 
              //host.ball['x']+=avan 
              //host.ball['x1']+=avan 
              //for(var c=1;c<tarjeta.length;c++){
                  //host.ball['x'+(c)]+=avan
                  //tarjeta[c].x+=host.ball['x']+0*c
              //}
          //}else{
         // if(onoff){
              host.ball['x']-=avan
    ...
    */
    ...
    dibujaParticipantes() 
    if (proyectorPrev != shared.onoffProyector) {
    ...
    //if(shared.log.length!=sharedPrev.length || (idvideoPrev != shared.idvideo && shared.modo=='video')){ //? && shared.modo==''
    //if(shared.log!=sharedPrev){ //? && shared.modo==''
    if (shared.log.length != sharedPrev.length) { //? && shared.modo==''
      ...
      //console.log('CARGADA API?: ' +shared.modo +' '+isloadediframe)
      if (shared.modo == 'video' && isloadediframe == -1) {
        //selectid.html('modo:'+shared.modo+'<br>'+'idvideo:'+shared.idvideo+'<br>'+shared.log)
    ...
    if(shared.log.length!=sharedPrev.length ){ //? && shared.modo==''
       var s='<div id="iframevideoyout"></div>'
        if(shared.log==s && shared.modo=='video'){pizarradiv.html(shared.log) ;isloadediframe=-1;sharedPrev=shared.log; return}
          decideTexto(shared.log);
          isloadediframe=-1
          selectid.html('modo:'+shared.modo+'<br>'+'idvideo:'+shared.idvideo+'<br>'+shared.log+':<br> '+shared.log.length+'<hr>Es diferente? <br> '+sharedPrev.length)
          //sharedPrev=shared.log.length
         sharedPrev=shared.log
    ...
    */
    ...
    //fill(33)
    //rect(0, suelo, width, height / 3);

    //----> dibujaTarjetas()

    //fill(255)
    /*text(
      `Mueven: ${participants.length}`,
      15,
      height - 50
    );
    */
  ...
  /*for (const p of participants) {
    if (typeof p.x !== "undefined" && typeof p.id !== "undefined") {
       let colorsilla=230
      ...
      //stroke(0)
      //line(vertice.x,vertice.y,p.x,p.y)
     
      //-->ellipse(p.x, p.y, 20, 20);
       fill(colorsilla)
      rect(partiNames[p.id].silla.x,partiNames[p.id].silla.y,partiNames[p.id].silla.w,partiNames[p.id].silla.h)
      fill(colorparti)
      textSize(14)
      text(partiNames[p.id].name,p.x,p.y)
...
function dibujaParticipantes() {
  ...
  if(shared.onoffProyector==1 && pizarradiv.html()==''){
      ...
      noFill()
      strokeWeight(30)
      ellipse(centerg.x,centerg.y,width/3,width/3)
      fill(255,60)
      strokeWeight(10)
      ellipse(centerg.x,centerg.y,20,20)
      noStroke()
      if(partiNames[me.id].rol==2){
      ...
      var distanceBC=dist(centerg.x, centerg.y, host.ball.x, host.ball.y)
  ...
  for (const m of participants) {
   ...
   // PANTALLA PROFE
      if (partiNames[m.id].rol == 2 && me.id == m.id &&  m.id>-1) {
        ...
        fill(255)
        textSize(tsprofe)
        text(tcopa, partiNames[m.id].elige[0].x, partiNames[m.id].elige[0].y + tsprofe)
        text(tmicro, partiNames[m.id].elige[1].x, partiNames[m.id].elige[1].y + tsprofe)
      ...
      */
      ...
      fill(0)
      textSize(14)
      text(partiNames[m.id].name, x + 10, y + 20)
      //PIDE
  ...
  if (shared.onoffProyector == 1) {
    ...
    if (shared.modo == 'imagen') {
      ...
      var ele = document.getElementById('nuestraimagen');
       if(ele){
          //yourImg.width = '800px';
          //ele.setAttribute('width', 800);
         //select('#nuestraimagen').attribute('width','800')
         select('#nuestraimagen').size(800,AUTO)
           pizarradiv.style('z-index', -1)
           //shared.log=select('#nuestraimagen').html()
            select('#nuestraimagen').style('z-index', 1)
       }

    }
    //pizarradiv.style('background-color', colorFondoT);
    ...
    if (select('#' + iframeid)) {
      ...
      //iframevideo.attribute('width', '800');
      //iframevideo.attribute('height', '500');
      if (shared.modo == 'video' && isloadediframe == 1) {
        //document.querySelector('#iframevideoyout').contentDocument.body.focus(); 

        player.setSize(800, 500)
        //document.querySelector('#pizarradiv').contentDocument.body.focus(); 
        //document.querySelector('#canvas').contentDocument.body.focus(); 
        //document.getElementById("iframevideoyout").focus();
        //player.playVideo()
      }
      pizarradiv.style('z-index', -1)

      //select('#nuestraimagen').attributte('width','800px')
      //document.getElementById("nuestraimagen").style.width = '800px'
      // var c = document.getElementById("pizarradiv").children;
      //for(var i=0;i<c.length;i++){  c[i].style.zIndex = "0";}
      cnv.style('z-index', 8000)
    }
  } else {
    pizarradiv.position(cnv.x + pizarra.x, cnv.y + pizarra.y);
    pizarradiv.size(pizarra.w, pizarra.h);
     ...
     if (shared.modo == 'imagen') {
       ...
       if(ele){
         ...
         select('#nuestraimagen').size(AUTO,pizarra.h)
           pizarradiv.style('z-index', 600)
           //shared.log=select('#nuestraimagen').html()
            select('#nuestraimagen').style('z-index', 601)
       }
     }
    if (partiNames[me.id].rol == 2) {
      button.show()
...
function repartePremios() {
  for (const m of participants) {
        if (m.premio == 1  && m.id > -1) { 
              ...
              text(tcopa, partiNames[m.id].elige[0].x, partiNames[m.id].elige[0].y + 14)
              //console.log(m.id)
        }
  }
}
function repartePalabra() {
  for (const m of participants) {
    if (m.palabra == 1 && m.id>-1) { 
              fill(255)
              textSize(partiNames[m.id].elige[0].w * 0.7)
              text(tmicro, partiNames[m.id].elige[1].x, partiNames[m.id].elige[1].y + 14)
              //console.log(m.id)
         }
...
function mostrar() {
  ...
  */
  ...
  selectid2.html('' + fecha + '<p>' + cad + '</p><br>', true)
  ...
  if (!iniciado) {
    ...
    if (typeof me.id !== "undefined") {
      ...
      if ((me.x > partiNames[me.id].silla.x) && (me.x < partiNames[me.id].silla.x + partiNames[me.id].silla.w) &&
        (me.y > partiNames[me.id].silla.y) && (me.y < partiNames[me.id].silla.y + partiNames[me.id].silla.h)) {
        me.atento = 8
...
function record(url, result) {
  ...
  if (result === true) {
    borratexto()
    solve = '<img id="nuestraimagen" src="' + url + '" height="' + pizarra.h + 'px" />'
    carga(solve, 'imagen');
    shared.idvideo = '';
    return
...
function votaPor(este) {
  if (me.voto == -1) {
    ...
    //pizarradiv.html('<br>'+opcionesVotar[int(e[1])].name+': '+opcionesVotar[int(e[1])].votos+' votos',true)
    //pizarradiv.html(este,true)
    //pizarradiv.html(e.join('  ')+'<br>',true)
    /* --> var se=select('#candiResult'+int(e[1]));
     se.html(shared.opcionesVotar[int(e[1])].votos)
     var cad=pizarradiv.html()
     shared.log=cad
     */
  }
}
...
function XXonPlayerReady(event) {
  ...
  /*
          -1 (unstarted, sin empezar)
           0 (ended, finalizado)
          1 (playing, en reproducci贸n)
          2 (paused, en pausa)
          3 (almacenando en b煤fer)
...
function yout(url) {
  ...
  if (videoid != null) {
    //var s='<iframe id="iframevideoyout" width="'+200+'" height="'+320+'" style="" src="https://www.youtube.com/embed/'+videoid[1]+'" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'


    //return s
    //var texto='<div id="iframevideoyout" ></div>'
    // var texto='<div id="player" ></div>'
    // 2. This code loads the IFrame Player API code asynchronously.

    //shared.log=s 
    //sharedPrev=shared.log
    //document.getElementById("pizarradiv").innerHTML = texto
    //pizarradiv.html(s) 
    //shared.modo='video'
    //shared.modoestado=-1
...
function pegadaImagen() {
 ...
 //borratexto()
    //var solve = '<img id="nuestraimagen" src="" height="' + pizarra.h + 'px" />'
 ...
 var clipboardData = event.clipboardData  || event.originalEvent.clipboardData;
  ...
  if (blob !== null) {
    ...
    reader.onload = function(event) {
      ...
      //document.getElementById("nuestraimagen").src = event.target.result;
     //shared.log = pizarradiv.html()
    };
